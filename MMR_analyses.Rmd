...
opts_chunk$set(comment=NA, echo=F, cache=T, tidy=F, message=F, warning=F, dev=c('png', 'pdf'), dpi=300, results='hide', fig.align = 'center', fig.height=8.5, fig.width=8.5)
options(width=100)
...
- This file contains figures and analyses presented in ``Ecological inference for infectious disease data, with application to vaccination strategies", along with additional analyses that are not discussed in detail in the primary manuscript. 
- There are three MCMC's run through Stan in this analysis, each may take approximately 30 minutes to compile. 
...
```{r epicurve, fig.height=4.5, fig.width=6.5}
...
```{r summap, fig.height=6.5, fig.width=6.5}
par(mar=c(0, 1, 0, 0))
...
```{r coverage_maps, fig.height=6.5, fig.width=6.5}
par(mar=c(0, 1, 0, 0))
...
```{r est_mle, results='markdown'}
ecomod=optim(par=c(1.5, 0, 0.5, 0.05, -0.5), fn=loglik_eco, y=measlesdat$y, xi=measlesdat$X, n=exp(measlesdat$logNi)*measlesdat$N, hessian=T, method='BFGS', control=list(reltol=1e-5))


se=sqrt(diag(solve(ecomod$hessian)))

data.frame(parameter=c('alpha_ar', 'phi', 'alpha_en', 'gamma', 'delta'), 
           Est=c(ecomod$par[1], expit(ecomod$par[2]), ecomod$par[3:5]), 
           SE=c(se[1], expit(se[2]), se[3:5]))
```


### Estimates from fixed effects Stan model

```{r fixed_effects_stan_mod, echo=T, results='markdown'}
...
```{r runstan_sens, echo=T, results='markup'}
