...
```{r sim_funs2, echo=F, eval=F}

##----------------------------------------------------------------------------##
#### Functions for simulations in the absence of vaccinnation (Section 5.1) ####
##----------------------------------------------------------------------------##
...
sim.pop=function(N, Nwks, AR, EN, y0=1, burnin=4, report_all=F){
        ...
        for(t in 2:(Nwks+burnin)){
                ...
                St[t]=ifelse(St[t-1]-Yt[t]<0, 0, St[t-1]-Yt[t])
...
sim.simple=function(N, Nwks, alpha_ar, alpha_en, ...){
        obs=sim.pop(N=N, Nwks=Nwks, AR=alpha_ar, EN=alpha_en, y0=1, ...)
        # fit each
...
##---------------------------------------------------------------------------------##
#### Functions for simulations in partially vaccinated populations (Section 5.2) ####
##---------------------------------------------------------------------------------##
...
sim.one=function(vacc='AoN', N, Nwks, alpha_ar, alpha_en, phi, x, sts=c(0.5, 0, 1)){
        ...
        if(vacc=='AoN'){
                obs=sim.AoN.pops(Npops=N, nwks=Nwks, AR=alpha_ar, EN=alpha_en, Phi=phi, X=x, y0=c(0, 0)) 
        }else{
                obs=sim.lk.pops(Npops=N, nwks=Nwks, AR=alpha_ar, EN=alpha_en, Phi=phi, X=x, y0=c(0, 0))
        }
...
#### Plots of Bias
rng.ar = range(res_r0big$Bias.ar)
...
rng.en = range(res_r0big$Bias.en)
...
#### Plots of Bias
rng.ar = range(res_r0mod$Bias.ar)
...
rng.en = range(res_r0mod$Bias.en)
...
```{r lk_sims, fig.height=4.5, fig.width=4.5, out.width='33%', fig.show = 'hold'}
...
res_asym = plyr::ldply(1:4, function(i){
        ...
        bias = rowMeans(apply(est, 2, function(x){(x - vals)}))
        mse = rowMeans(apply(est, 2, function(x){(x - vals)^2}))
        c(mod = i, Est.ar = ests[1], Bias.ar = bias[1], MSE.ar = mse[1], 
          Est.en = ests[2], Bias.en = bias[2], MSE.en = mse[2],
          Est.phi = ests[3], Bias.phi = bias[3], MSE.phi = mse[3])
        
})


## Plot Estimates
rng = with(res_asym, range(Est.ar, Est.en, Est.phi, 0))
rng.ar = range(res_asym$Est.ar, 0)
par(mar=c(3, 3.1, 0, 0)+0.2)
plot(x=c(0.5, 4.5), y=rng.ar, type='n', xaxt='n', ylab='', xlab='', xaxs='i')
mtext(expression(Est~ widehat(alpha[scriptscriptstyle(AR)])), side=2, line=2)
abline(h=0, col='red')
axis(1, at=1:4, labels=c('All-or-none', 'Leaky', 'Ecological', 'Epidemic-\nEndemic'), padj=0.5)
points(x=c(1:4), y=res_asym$Est.ar, pch=16 )

rng.en = range(res_asym$Est.en, 0)
plot(x=c(0.5, 4.5), y=rng.en, type='n', xaxt='n', ylab='', xlab='', xaxs='i')
mtext(expression(Est~widehat(alpha[scriptscriptstyle(EN)])), side=2, line=2)
abline(h=0, col='red')
axis(1, at=1:4, labels=c('All-or-none', 'Leaky', 'Ecological', 'Epidemic-\nEndemic'), padj=0.5)
points(x=1:4, y=res_asym$Est.en, pch=16 )

rng.phi = range(res_asym$Est.phi[1:3], 0)
plot(x=c(0.5, 3.5), y=rng.phi, type='n', xaxt='n', ylab='', xlab='', xaxs='i')
mtext(expression(Est~paste( widehat(phi))), side=2, line=2)
abline(h=0, col='red')
axis(1, at=1:3, labels=c('All-or-none', 'Leaky', 'Ecological'))
...
```{r ecobias_eg, eval=T}
x=seq(0.15, 0.75, by=0.15) 
N=rep(1e5, length(x))
sim.vals=c(-6, 5.25, -11)

tmp = sim.eco.bias(N=N, Nwks=52, x=x, a0=sim.vals[1], a1=sim.vals[2], a2=sim.vals[3], sts=c(-1, 0.5, -5))

matplot(t(tmp$dat$Y), type='l', lty=1)
tmp$agg$par-sim.vals
tmp$naive$par-sim.vals


# set.seed(5)
eco.b=replicate(1e3, sim.eco.bias(N=N, Nwks=052, x=x, a0=sim.vals[1], a1=sim.vals[2], a2=sim.vals[3])) 

rowMeans(sapply(eco.b['agg', ], function(x){x$par-sim.vals}))
rowMeans(sapply(eco.b['naive', ], function(x){x$par-sim.vals}))
...
```{r print_sim_funs,  ref.label='sim_funs', eval=F, echo=T}
